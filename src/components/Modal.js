...
const ModalContainer = styled.div`
	...
	> div {
		background-color: ${(props) =>
			props.darkMode
				? "var(--background-Form-Dark)"
				: "var(--background-Form-Light)"};
...
export default function Modal(props) {
	...
	const handleSubmit = (e) => {
		...
		setTask([
			...tasks,
			{ _id: uuidv4(), done: false, content: valueInput },
		]);
		setValueInput("");
	};
	...
	const clickTask = (task) => {
		...
		setTask([
			...tmp,
			{ _id: task._id, done: !task.done, content: task.content },
		]);
	};
		...
		<ModalContainer darkMode={props.darkMode} maxWidth={props.maxWidth}>
			...
			<div>
				...
				<section>
					<Filter
						left={leftTasks()}
						clearCompleted={clearDoneTasks}
						clickAll={clickAll}
						clickActivated={clickActivated}
